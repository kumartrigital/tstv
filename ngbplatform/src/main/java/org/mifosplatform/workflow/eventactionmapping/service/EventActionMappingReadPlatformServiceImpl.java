	...
	private static final class EventActionMapper implements
		...
		public String schema() {
			return "em.id as id, em.event_name as eventName, em.action_name as actionName, em.process as process,"
					+ "em.order_by as orderBy, em.pre_post as prePost,em.process_params as processParams,em.is_synchronous as issynchronous,"
					+ "em.is_deleted as isDeleted from b_eventaction_mapping em order by em.is_deleted";
		...
		public EventActionMappingData mapRow(final ResultSet rs,
			...
			Long orderBy = JdbcSupport.getLong(rs,"orderBy");
			String prePostValue = rs.getString("prePost");
			boolean prePost = false;
			if("Y".equalsIgnoreCase(prePostValue)){prePost=true;}
			String processParams = rs.getString("processParams");
			String isDeleted = rs.getString("isDeleted");
			Boolean issynchronous = rs.getBoolean("issynchronous");
			...
			return new EventActionMappingData(id, eventName, actionName,
					process, orderBy, prePost, processParams, isDeleted,issynchronous);
		}
	}

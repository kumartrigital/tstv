...
public class ClientParentApiResource {
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retriveParentChildrens(@PathParam("clientId") final Long clientId,@Context final UriInfo uriInfo) {
		
		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissions);
		final List<ClientCategoryData> parentClientData = this.clientReadPlatformService.retrievedParentsChild(clientId);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
		return this.toApiJsonSerializer.serialize(settings,	parentClientData, RESPONSE_DATA_PARAMETERS);
	}
}
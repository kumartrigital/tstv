...
public interface OrderRepository extends JpaRepository<Order, Long>, JpaSpecificationExecutor<Order> {
	...
	List<Order> findOrdersByClientService(@Param("clientServiceId") Long clientServiceId,
	...
	@Query(value ="select * from  b_orders where client_id=:clientId and plan_id=:planId and (order_status = 1 or order_status = 4)", nativeQuery = true)
	...
	@Query("from Order order where order.clientId=:clientId and order.status = 1")
	List<Order> findActiveOrdersOnlyByClientId(@Param("clientId") Long clientId);

	@Query("from Order order where order.orderNo=:orderNo")
	Order findOrderByOrderNO(@Param("orderNo") String orderNo);

	@Query(value = "select * from b_orders where id=:Id", nativeQuery = true)

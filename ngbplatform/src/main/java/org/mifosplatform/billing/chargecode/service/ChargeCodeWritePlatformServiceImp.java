...
public class ChargeCodeWritePlatformServiceImp implements ChargeCodeWritePlatformService {
    ...
    private final GenerateCharges generateBill;
	...
	public ChargeCodeWritePlatformServiceImp(final PlatformSecurityContext context,final ChargeCodeRepository chargeCodeRepository,
			...
			final PriceRepository priceRepository,final GenerateCharges generateBill,final ClientRepositoryWrapper clientRepository) {
	...
	public BigDecimal calculateFinalAmount(ChargeCodeData chargeCodeData,Long clientId,Long priceId) {
		...
		DateTime defaultDate= DateTimeUtils.getDateTimeOfTenant();
		...
		LocalDateTime endDate=new LocalDateTime(discountMaster.getStartDate()).plusMonths(1);
		...
		DateTime DiscountStartDate =  new DateTime(discountMaster.getStartDate());
		BillingOrderData billingOrderData = new BillingOrderData(defaultValue,defaultValue,defaultValue,clientId,DiscountStartDate,
				...
				chargeCode.getChargeType(),defaultDate,price.getPrice(),"",DiscountStartDate,defaultDate,defaultValue,chargeCode.getTaxInclusive(),
				String.valueOf(client.getTaxExemption()), false,Long.valueOf(price.getCurrencyId()),price.getChargeOwner()); 
		...
		List<ChargeTaxCommand> invoiceTaxCommands=this.generateBill.calculateDiscountAndTax(billingOrderData, discountMasterData, new LocalDateTime(discountMaster.getStartDate()),endDate, price.getPrice());

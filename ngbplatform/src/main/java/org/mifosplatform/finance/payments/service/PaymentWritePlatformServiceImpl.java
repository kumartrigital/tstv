...
public class PaymentWritePlatformServiceImpl implements PaymentWritePlatformService {
	...
	public PaymentWritePlatformServiceImpl(final PlatformSecurityContext context,final PaymentRepository paymentRepository,
			...
			final CreditDistributionRepository creditDistributionRepository,final  CrmServices crmServices,final ClientBillProfileInfoRepository clientBillProfileInfoRepository,final FromJsonHelper fromJsonHelper) {
	...
	public CommandProcessingResult createPayment(final JsonCommand command) {
		...
		try {
			...
			this.fromApiJsonDeserializer.validateForCreate(command.json());
			 CommandProcessingResult result=this.crmServices.createPayment(command);
			...
			ClientBillProfileInfo ClientBillProfileInfo=this.clientBillProfileInfoRepository.findwithclientId(payment.getClientId());
			Long clientServiceId = Long.valueOf(0);
			
			JSONObject clientBalanceObject = new JSONObject();
			try {
			clientBalanceObject.put("clientId",payment.getClientId());
			clientBalanceObject.put("amount", payment.getAmountPaid());
			clientBalanceObject.put("isWalletEnable", Character.valueOf(payment.isWalletPayment()));
			...
			ClientBalance clientBalance=this.clientBalanceRepository.findByClientAndClientServiceId(payment.getClientId(),Long.valueOf(0));			
			if(clientBalance != null){
				...
				clientBalance =ClientBalance.create(payment.getClientId(),balance,payment.isWalletPayment(),clientServiceId,ClientBillProfileInfo.getBillCurrency());
			}
	...
	public CommandProcessingResult cancelPayment(final JsonCommand command,final Long paymentId) {
		...
		try{
			...
			this.crmServices.cancelPayment(command);
           ...
           final Payment cancelPay=new Payment(payment.getClientId(), null, null, payment.getAmountPaid(), null, DateUtils.getLocalDateOfTenant(),payment.getRemarks(), 
			...
			if(listOfInvoices != null){
				...
				// Notify Payment Cancel Payments
				this.orderWritePlatformService.processNotifyMessages(EventActionConstants.EVENT_CREATE_PAYMENT_REVERSAL, payment.getClientId(), payment.getAmountPaid().toString(), null);	
			}

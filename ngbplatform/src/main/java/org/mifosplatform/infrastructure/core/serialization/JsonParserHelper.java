...
public class JsonParserHelper {
    ...
    public LocalDateTime extractLocalDateTimeNamed(final String parameterName, final JsonElement element,
        ...
        if (element.isJsonObject()) {
            ...
            final String dateFormat = extractDateFormatParameter(object);
            final Locale clientApplicationLocale = extractLocaleParameter(object);
            value = extractLocalDateTimeNamed(parameterName, object, dateFormat, clientApplicationLocale, parametersPassedInCommand);
        }
        return value;
    }
    ...
    public LocalDateTime extractLocalDateTimeNamed(final String parameterName, final JsonObject element, final String dateFormat,
            final Locale clientApplicationLocale, final Set<String> parametersPassedInCommand) {
        LocalDateTime value = null;
        if (element.isJsonObject()) {
            JsonObject object = element.getAsJsonObject();
            ...
            if (object.has(parameterName) && object.get(parameterName).isJsonPrimitive()) {
                ...
                final String valueAsString = primitive.getAsString();
                if (StringUtils.isNotBlank(valueAsString)) {
                    value = convertTimeFrom(valueAsString, parameterName, dateFormat, clientApplicationLocale);
                }
            }
    ...
    public static LocalDateTime convertTimeFrom(final String dateAsString, final String parameterName, final String dateFormat,
            final Locale clientApplicationLocale) {
        ...
        LocalDateTime eventLocalDate = null;
        if (StringUtils.isNotBlank(dateAsString)) {
            try {
                // Locale locale = LocaleContextHolder.getLocale();
                eventLocalDate = DateTimeFormat.forPattern(dateFormat).withLocale(clientApplicationLocale)
                        .parseLocalDateTime(dateAsString.toLowerCase(clientApplicationLocale));
            } catch (IllegalArgumentException e) {
                List<ApiParameterError> dataValidationErrors = new ArrayList<ApiParameterError>();
                ApiParameterError error = ApiParameterError.parameterError("validation.msg.invalid.date.format", "The parameter "
                        + parameterName + " is invalid based on the dateFormat: '" + dateFormat + "' and locale: '"

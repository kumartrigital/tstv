...
public class OfficePaymentsApiResource {
	...
	public String createOfficePayment(@PathParam("officeId") final Long officeId, final String apiRequestBodyAsJson) {
	 ...
	 @Consumes({ MediaType.APPLICATION_JSON })
	 @Produces({ MediaType.APPLICATION_JSON })
	 public String cancelofficepayment(@PathParam("paymentId") final Long paymentId,final String apiRequestBodyAsJson) {
		 
			final CommandWrapper commandRequest = new CommandWrapperBuilder().cancelofficepayment(paymentId).withJson(apiRequestBodyAsJson).build();
			final CommandProcessingResult result = this.writePlatformService.logCommandSource(commandRequest);
			return this.toApiJsonSerializer.serialize(result);
		}	

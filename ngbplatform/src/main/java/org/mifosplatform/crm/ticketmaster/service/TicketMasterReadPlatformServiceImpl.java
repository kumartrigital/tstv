...
public class TicketMasterReadPlatformServiceImpl  implements TicketMasterReadPlatformService{
	...
	public Page<ClientTicketData> retrieveAssignedTicketsForNewClient(SearchSqlQuery searchTicketMaster, String statusType,
       ...
       /* if(fromDate != null && toDate != null) {
        	sqlBuilder.append(" and tckt.ticket_date between '"+fromDate+"' and '"+toDate+"' ");
	    }*/
	    if (sqlSearch != null) {
	    	...
	    	extraCriteria = " and ((select display_name from m_client where id = tckt.client_id) like '%"+sqlSearch+"%' OR" 
	    			...
	    			+ " tckt.status like '%"+sqlSearch+"%' OR tckt.ticket_date like '%"+sqlSearch+"%' OR"
	    ...
	    if(fromDate != null && toDate != null && statusType != null) {
        	sqlBuilder.append(" and tckt.status = '"+statusType+"' and DATE_FORMAT(tckt.ticket_date,'%Y-%m-%d') between '"+fromDate+"' and '"+toDate+"' ");
	    }
	    /*if(statusType != null){
	    	extraCriteria =" or tckt.status='"+statusType+"'";
	    }*/
	...
	private static final class ClientTicketMappe implements RowMapper<TicketMasterData> {
		...
		public TicketMasterData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			return new TicketMasterData(id, priority, status, userId, ticketDate, LastComment, problemDescription, assignedTo, sourceOfTicket,
					dueDate, description, resolutionDescription, problemCode, statusCode,ticketstatus,DocumentId,fileName);
		}
	}
	...
	private static final class ClientTicketMapper implements RowMapper<TicketMasterData> {
        ...
        return  " btm.id AS id, btm.priority AS priority,btm.ticket_date AS ticketDate,btm.problem_code AS problemCode, btm.status AS status,btm.sub_category AS subCategoryes,"+
		  ...
		  /*return " btm.id AS id, btm.priority AS priority,btm.ticket_date AS ticketDate,"+
		...
		public TicketMasterData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			return new TicketMasterData(id, priority, status, null, ticketDate, LastComment, null, null, null,
					dueDate, null, null, problemCode, null,null,teamCode,teamUser,teamUserId,teamId,subCategoryes);
		}
	}
	...
	public TicketMasterData retrieveSingleTicketDetails(final Long clientId, final Long ticketId) {
		try {
				...
				final String sql = "select " + mapper.clientOrderLookupSchema()+" and btd.ticket_id='"+ticketId+"' and btm.client_id='"+clientId+"'" ;
	...
	private static final class TicketDataMapper implements
		...
		public TicketMasterData mapRow(ResultSet resultSet, int rowNum)
			...
			final TicketMasterData data = new TicketMasterData(id, createdDate, assignedTo, description, fileName,teamCode);
	...
	private static final class UserTicketsMapperForNewClient implements RowMapper<ClientTicketData> {
		...
		public String userTicketSchema() {
			...
			"left join m_office o on o.id = mct.office_id "+
			"UNION ALL "+
			"select tckt.id AS id,tckt.office_id AS clientId,o.name AS clientName,tckt.priority AS priority,tckt.status AS status, "+ 
		    "tckt.ticket_date AS ticketDate,tckt.closed_date AS closedDate, "+
		    ...
		    "FROM b_office_ticket tckt LEFT JOIN m_office o ON o.id = tckt.office_id ";*/
		...
		public ClientTicketData mapRow(ResultSet resultSet, int rowNum) throws SQLException {
			...
			ClientTicketData clientticketData =new ClientTicketData(id, priority, status, userId, ticketDate, lastComment, problemDescription,
	...
	public List<TicketMasterData> retrieveOfficeTicketDetails(final Long officeId) {
		try {
				...
				final String sql = "select " + mapper.officeOrderLookupSchema() + " and btm.office_id= ? order by btm.id DESC ";
	...
	public TicketMasterData retrieveSingleOfficeTicketDetails(final Long officeId, final Long ticketId) {
		try {
				...
				final String sql = "select " + mapper.officeOrderLookupSchema() + " and btm.office_id= ? and btm.id=? order by btm.id DESC ";
	...
	private static final class OfficeTicketMapper implements RowMapper<TicketMasterData> {
		...
		*/
		...
		public TicketMasterData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			TicketMasterData ticketMasterData= new TicketMasterData(id, priority, status, null, ticketDate, LastComment, problemDescription, assignedTo, sourceOfTicket,
					dueDate, description, resolutionDescription, problemCode, statusCode,null,null,null,null,null,null);
	   ...
	   private class TicketHistoryMapper implements RowMapper<TicketMasterData> {
		...
		public TicketMasterData mapRow(ResultSet rs, int rowNum) throws SQLException {
		    ...
		    return new TicketMasterData(id, priority, ticketDate, problemDescription, status, subCategorys,Comment_date,comments,teamCode,assignedTo);
		}
	...
	private static final class TicketMapperForOffice implements RowMapper<TicketMasterData> {
		...
		public TicketMasterData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			TicketMasterData ticketMasterData= new TicketMasterData(id, clientId,priority, status, null,problemDescription, ticketDate,assignedTo,
					sourceOfTicket,dueDate,description,resolutionDescription,problemCode,subcategory,commentedDate,closedDate,createdBy);
			return ticketMasterData;
		}
	}

...
public class ClientProspectCommandFromApiJsonDeserializer {
	...
	private final Set<String> supportedParameters = new HashSet<String>(Arrays.asList("id", "sourceOfPublicityInt", "preferredPlanInt",
					...
					"address", "streetArea", "cityDistrict", "state", "country", "locale", "preferredPlan", "status","officeId",
					"statusRemark", "callStatus", "assignedTo", "notes", "isDeleted", "zipCode","district","service","Prospect","preferedService","arrOfStr")); /*added district*/
	...
	public void validateForCreate(final String json) {
		...
		baseDataValidator.reset().parameter("district").value(district).notBlank().notExceedingLengthOf(100);
		
		final Integer officeId = fromApiJsonHelper.extractIntegerWithLocaleNamed("officeId", element);
		baseDataValidator.reset().parameter("officeId").value(officeId).notBlank().notExceedingLengthOf(20);

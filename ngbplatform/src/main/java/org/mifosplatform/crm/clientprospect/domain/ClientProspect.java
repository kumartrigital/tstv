...
public class ClientProspect extends AbstractAuditableCustom<AppUser, Long> {
	...
	public ClientProspect(final Short prospectType, final String firstName,
			...
			final String status, final String statusRemark, final String zipCode,final String preferedservice) {
		this.prospectType = prospectType;
		this.firstName = firstName;
		this.middleName = middleName;
		this.lastName = lastName;
	...
	public ClientProspect(final Short prospectType, final String firstName,
			...
			final String note, final String address, final String streetArea,
			final String cityDistrict, final String state,
			final String country, final String zipCode,final String district) {
		this.prospectType = prospectType;
		this.firstName = firstName;
		this.middleName = middleName;
		this.lastName = lastName;
...
/*
	...
	public static ClientProspect fromJson(final FromJsonHelper fromJsonHelper,
		...
		if (fromJsonHelper.parameterExists("city", element)) {
			String cityDistrict = command.stringValueOfParameterNamed("city");
		...
		/*
		added district*/
		if (fromJsonHelper.parameterExists("district", element)) {
			String district = command.stringValueOfParameterNamed("district");
			clientProspect.setDistrict(district);
		}
		
		if (fromJsonHelper.parameterExists("service", element)) {
			JsonArray preferedservice = command.arrayOfParameterNamed("service");
			clientProspect.setPreferedservice(preferedservice.toString());
		}
	...
	public Map<String, Object> update(final JsonCommand command) {
		...
		final String preferedservice = "preferedservice";
		...
		/*
		added district*/
		if (command.isChangeInStringParameterNamed(district, this.district)) {
			final String newValue = command.stringValueOfParameterNamed("district");
			actualChanges.put(district, newValue);
			this.district = newValue;
		}
		
		if (command.isChangeInStringParameterNamed(preferedservice, this.preferedservice)) {
			final String newValue = command.stringValueOfParameterNamed("preferedservice");
			actualChanges.put(preferedservice, newValue);
			this.preferedservice = newValue;
		}
	...
	/*public Map<String, Object> elevateupdate(JsonCommand command) {

		Map<String, Object> actualChanges = new LinkedHashMap<String, Object>(1);
		
		final String status = "status";
		
		if (command.isChangeInIntegerParameterNamed(status, Integer.valueOf(this.status))) {	

  ...
  public String retriveSingleItemDetail(@Context final UriInfo uriInfo, @PathParam("itemId") final Long itemId) {
   ...
   return this.toApiJsonSerializerForItem.serialize(settings,clientDatafinal,RESPONSE_ITEM_DETAILS_PARAMETERS);
	...
	public String retriveItemSerialNumbers(@PathParam("itemmasterId") final Long itemmasterId,@PathParam("officeId") final Long officeId,
			@QueryParam("query") final String query,@QueryParam("pairedItemId") final Long pairedItemId, @Context final UriInfo uriInfo){
			
		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissionsAllocation);
		List<String> itemSerialNumbers = this.itemDetailsReadPlatformService.retriveSerialNumbersOnKeyStroke(itemmasterId,query,officeId,pairedItemId);
		...
		{
			throw new SerialNumberNotFoundException(query);
		}
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
	...
	public String retriveGrnIds(@Context final UriInfo uriInfo){
		...
		Collection<ModelProvisionMappingData> modelProvisionMappingData = this.modelProvisionMappingReadPlatformService.retrieveAllForDropDown();
		ItemDetailsData itemDetailsData=new ItemDetailsData(inventoryGrnData,qualityDatas,statusDatas,null,null,modelProvisionMappingData);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
	...
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public String retriveSwapReason(@Context final UriInfo uriInfo){
		
		context.authenticatedUser().validateHasReadPermission(resourceNameForGrnPermissions);
		Collection<MCodeData> swapDevicesReasons=this.mCodeReadPlatformService.getCodeValue(CodeNameConstants.SWAP_DEVICE_REASONS);
		ItemDetailsData itemDetailsData=new ItemDetailsData(null,null,null,null,null,null);
		itemDetailsData.setSwapDevicesReasons(swapDevicesReasons);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
		return this.toApiJsonSerializerForItem.serialize(settings,itemDetailsData,RESPONSE_DATA_GRN_IDS_PARAMETERS);
	}
	...
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public String retriveItemDetailsBySerialNum(@QueryParam("query") final String query,@QueryParam("clientId") final Long clientId,@QueryParam("officeId") final Long officeId, @Context final UriInfo uriInfo){
		
			 context.authenticatedUser().validateHasReadPermission(resourceNameForPermissionsAllocation);
			 final ItemData itemMasterData = this.itemDetailsReadPlatformService.retriveItemDetailsDataBySerialNum(query,clientId,officeId);
	...
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public String transferItems(final String apiRequestBodyAsJson) throws JSONException{
			
		Integer finalResult = this.itemDetailsReadPlatformService.updateItemDetail(apiRequestBodyAsJson);
		return this.toApiJsonSerializerForAllocationHardware.serialize(finalResult);
	}
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String swapDevice(final String apiRequestBodyAsJson) {
		
		 final CommandWrapper commandRequest = new CommandWrapperBuilder().swapDevice().withJson(apiRequestBodyAsJson).build();
		 final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
		  return this.toApiJsonSerializerForItem.serialize(result);
	}

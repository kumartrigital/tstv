...
public class MRNDetailsWritePlatformServiceImp implements MRNDetailsWritePlatformService{
	...
	public MRNDetailsWritePlatformServiceImp(final MRNDetailsJpaRepository mrnDetailsJpaRepository,final PlatformSecurityContext context,
			...
			final ItemSaleRepository itemSaleRepository,final VoucherReadPlatformService voucherReadPlatformService) {
	...
	public CommandProcessingResult movemrncarton(JsonCommand command) {
		...
		try {
			...
			if(!cartoonNumber.contains(cartoonNumber)){
				throw new PlatformDataIntegrityException("invalid.cartonNumber.allocation", "invalid.cartonNumber.allocation", "cartoonNumber","");
			}
			...
			if(itemDetail.getcartoonNumber().equalsIgnoreCase(cartoonNumber)){
					...
					InventoryTransactionHistory transactionHistory = InventoryTransactionHistory.logTransaction(mrnDetails.getRequestedDate(), mrnId,"MRN", cartoonNumber,mrnDetails.getItemMasterId(),
			                mrnDetails.getFromOffice(), mrnDetails.getToOffice());
				...
				throw new PlatformDataIntegrityException("cartoonNuber.is.not.valid", "cartoonNuber.is.not.valid", "cartoonNuber.is.not.valid");	
			}	
			}
